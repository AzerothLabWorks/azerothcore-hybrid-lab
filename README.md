...
- **Multi-resource hybrid support**: QA server config can grant baseline mana, energy, and rage pools so cross-class builds can test abilities that depend on non-native class resources. Hidden resource pools are refreshed after login power restore for existing hybrid characters.
...
- **Any-race/any-class creation data**: QA startup data includes fixes for unsupported race/class combinations, including Blood Elf Warriors receiving Arcane Torrent.
...
- **QA Azeroth flying unlock**: The QA branch includes config-gated flying support for old-world Azeroth, including server-side support for Eversong Woods, Ghostlands, Azuremyst Isle, and Bloodmyst Isle. The Burning Crusade starter zones are usable for flying but remain a known client terrain edge case; deeper ADT terrain patching is intentionally deferred.
