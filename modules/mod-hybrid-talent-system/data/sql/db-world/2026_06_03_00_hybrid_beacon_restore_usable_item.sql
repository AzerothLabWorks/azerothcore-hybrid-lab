...
CREATE TEMPORARY TABLE `tmp_hybrid_beacon_item_restore`
SELECT *
FROM `item_template`
WHERE `entry` = 6948
LIMIT 1;

UPDATE `tmp_hybrid_beacon_item_restore`
SET
    `entry` = @HYBRID_BEACON,
    `class` = 15,
    `subclass` = 0,
...
SELECT *
FROM `tmp_hybrid_beacon_item_restore`;

DROP TEMPORARY TABLE `tmp_hybrid_beacon_item_restore`;
